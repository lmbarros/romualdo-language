...
* Add an `OpJumpIfFalseLong` instruction. When patching a jump that jumps longer
  than what fits in a signed byte, we'll need to replace it with the long
  version, and move things around in the generated bytecode to free up space for
  the longer jump offset. (Shall be OK, since jumps are all relative and the
  language does not support things like arbitrary gotos that could break this
  scheme.)
